...
- ~~**待办**：issue #4~~ **已完成（2026-07-26，PR #6 合并）**：卡片/占位卡追加到底部与字幕顺序一致，`_trimFeedback` 删顶端最旧；字幕区与卡片区均「距底 <80px 才跟随滚动」（`_isNearBottom`），回看旧内容不被拉走。PR 基于 v2 前代码，合并时把语义重新表达到 v2 结构（插入点在 `_insertPendingCard` 与批次 `replaceWith` 处）。
