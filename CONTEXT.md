...
|---|---|---|---|---|
| **Mode A · 表达纠错** | 说**英语** | 英语→文字 | 指出哪里不正确 / 不地道（语法、搭配、Chinglish） | 🔨 开发中（切片 1 起） |
| **Mode B · 中译英学习** | 说**中文** | 中文→文字 | 翻译成**地道英文** + 讲解 + **跟读**练习 | ⏳ 待做（切片 5-6） |
...
1. **骨架切换**（本切片）：开分支、改名、拆中文情绪词库、`lexicon.js` 三表英语化 → Mode A 本地高亮跑通。
2. **A-纠错卡**：按句调 AI + 结构化返回（含标签）+ 卡片 UI。
3. **A-报告**：新 6 维英语报告 prompt。
4. **数据层**：JSON + MD + INDEX + 标签注册表。
5. **B-翻译卡**：模式切换 UI + 中→英学习卡。
6. **B-跟读环**：跟读状态机 + 词级 diff。
7. **打磨**：文案、prompt 调优。
