...
/**
 * Mode A — per-sentence correction prompt.
 * @param {string} sentence   one ASR-transcribed English sentence
 * @param {string[]} existingTags  tags already seen this session — reuse first
 * @returns {{system: string, user: string}}
 */
function getCorrectionPrompt(sentence, existingTags = []) {
  const tagBlock = existingTags.length
    ? `\n\n已有标签（请优先从中选择，只有确实不合适时才新建标签）：\n${existingTags.join(' / ')}`
    : '';

  return {
    system: `你是英语表达教练。用户在做英语口语训练，你会收到**一句**语音识别（ASR）转写的英文。判断它在**语法 / 搭配 / 地道性**上是否有问题，并给出修正。
...
如果有问题：
{"hasError": true, "original": "原句", "corrected": "修正后的地道说法", "explanation": "用中文一句话说明为什么", "tags": ["标签1", "标签2"]}
...
/**
 * Mode B — Chinese → idiomatic English learning card.
 * @param {string} sentence   one ASR-transcribed Chinese sentence
 * @param {string[]} existingTags  tags already seen this session — reuse first
 * @returns {{system: string, user: string}}
 */
function getTranslationPrompt(sentence, existingTags = []) {
  const tagBlock = existingTags.length
    ? `\n\n已有标签（优先复用，不合适才新建）：\n${existingTags.join(' / ')}`
    : '';

  return {
    system: `你是中译英学习教练。用户在做「说中文 → 学地道英文」训练，你会收到**一句**语音识别转写的中文。把它翻成**地道、自然的口语英文**，并给出简短讲解。

## 输出：严格 JSON（不要 markdown，不要代码块，不要多余文字）
{"zh": "整理后的中文原句", "en": "地道英文说法", "note": "中文讲解", "tags": ["标签"]}

## 规则
- en：母语者会说的地道口语，**不是逐字直译**；如果有更自然的说法，优先用它。
- note：用**中文**，点出 1-2 个关键学习点（关键词汇 / 搭配 / 句型 / 为什么这样说），不超过 50 字。
- zh：把 ASR 可能的口语碎片整理成通顺的中文原句。
- tags：1-3 个中文，描述**场景或知识点**（如：场景-日常 / 场景-工作 / 句型-条件句 / 短语动词 / 时态 / 地道表达）。${tagBlock}
- 只输出 JSON 本身。`,
