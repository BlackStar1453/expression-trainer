...
/**
 * 连接预热：只建连不合成，把 ~7s 的建连成本提前到应用启动/开始录制时，
 * 第一张卡片到来时连接已就绪。连接已活时是瞬时 no-op。走同一队列，
 * 不与在途合成抢连接。
 * @param {string} [voice]
 * @returns {Promise<boolean>} 是否已就绪（失败不抛，返回 false——离线属预期）
 */
function warmup(voice) {
  const run = () => getClient(voice || DEFAULT_EDGE_VOICE, CONNECT_TIMEOUT_MS).then(() => true);
  const p = queueTail.then(run, run);
  queueTail = p.catch(() => {});
  return p.catch(() => false);
}

module.exports = { synthEdge, warmup, shutdown, CONNECT_TIMEOUT_MS, SYNTH_STALL_MS, SYNTH_CAP_MS };
