...
(function (factory) {
  ...
  /**
   * Escape a string for embedding in SSML/XML (element text or attribute).
   * msedge-tts inserts the input verbatim into its SSML request, so bare
   * `&` / `<` (e.g. "R&D") would otherwise make the XML malformed and Edge
   * reject the synth — mis-read by the fallback path as a network failure.
   */
